...
#ifdef WIN32
const char* iconv(unsigned int from, unsigned int to, const char* str,
                  size_t sz)
{
...
static int quit_f(int argc, char** argv) { return 1; }
...
#ifdef WITH_MOZJS
static char* hash_load_js(const char* str1, const char* str2,
                          LwqqHttpRequest* req)
{
...
int main(int argc, char* argv[])
{
   ...
   const struct option long_options[] = { { "version", 0, 0, 'v' },
                                          { "help", 0, 0, 'h' },
                                          { "user", 0, 0, 'u' },
                                          { "pwd", 0, 0, 'p' },
                                          { 0, 0, 0, 0 } };
   ...
   while ((c = getopt_long(argc, argv, "vhu:p:", long_options, NULL)) != EOF) {
      switch (c) {
      case 'v':
   ...
   lwqq_add_event(lc->events->poll_msg, _C_(p, received_msg, lc->msg_list));
#ifdef WITH_MOZJS
   LwqqHttpRequest* req
       = lwqq_http_request_new("http://pidginlwqq.sinaapp.com/hash.js");
