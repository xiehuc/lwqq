...
typedef int (*LwqqProgressFunc)(void* data, size_t now, size_t total);
/************************************************************************/
...
typedef enum {
   LWQQ_DEL_KEEP_OTHER
   = 1, /* delete buddy and keep myself from other buddy list */
   LWQQ_DEL_FROM_OTHER
   = 2 /* delete buddy and remove myself from other buddy list */
} LwqqDelFriendType;
typedef enum { LWQQ_GROUP_QUN = 0, LWQQ_GROUP_DISCU = 1 } LwqqGroupType;
typedef enum { LWQQ_CLEANUP_WAITALL = 0, LWQQ_CLEANUP_IGNORE = 1 } LwqqCleanUp;
...
typedef LIST_HEAD(LwqqFriendList, LwqqBuddy) LwqqFriendList;
...
struct LwqqClient {
   ...
   LwqqBuddy* (*find_buddy_by_qqnumber)(struct LwqqClient* lc,
                                        const char* qqnumber);
...
#define lwqq_client_valid(lc)                                                  \
   (lc != 0 && ((LwqqClient*)lc)->magic == LWQQ_MAGIC)
...
LwqqBuddy* lwqq_buddy_find_buddy_by_qqnumber(LwqqClient* lc,
                                             const char* qqnumber);
/**
...
LwqqFriendCategory* lwqq_category_find_by_name(LwqqClient* lc,
                                               const char* name);
...
#define lwqq_discu_find_discu_by_did(lc, did)                                  \
   lwqq_group_find_group_by_gid(lc, did);
LwqqGroup* lwqq_group_find_group_by_qqnumber(LwqqClient* lc,
                                             const char* qqnumber);
#define lwqq_group_find_group_by_account(lc, acc)                              \
   lwqq_group_find_group_by_qqnumber(lc, acc)
 ...
 * @return A LwqqBuddy instance
 */
LwqqSimpleBuddy* lwqq_group_find_group_member_by_uin(LwqqGroup* group,
                                                     const char* uin);
...
void lwqq_hash_add_entry(LwqqClient* lc, const char* name, LwqqHashFunc func,
                         void* data);
