...
{
   LwqqSimpleBuddy* ret
       = ((LwqqSimpleBuddy*)s_malloc0(sizeof(LwqqSimpleBuddy)));
...
LwqqFriendCategory* lwqq_category_find_by_name(LwqqClient* lc, const char* name)
{
   ...
   {
      if (cate->name && !strcmp(cate->name, name))
         return cate;
   }
   return NULL;
}
                                             ...
                                             const char* qqnumber)
{
   ...
   LIST_FOREACH(group, &lc->groups, entries)
   {
      if (group->account && !strcmp(group->account, qqnumber))
         return group;
   }

   LIST_FOREACH(discu, &lc->discus, entries)
   {
      if (discu->account && !strcmp(discu->account, qqnumber))
         return discu;
   }
   return NULL;
}
...
char* lwqq_hash_auto(const char* uin, const char* ptwebqq, void* lc)
{
   ...
   if (lc_->hash_next) {
      lc_->hash_idx++;
      if (lc_->hash_idx->name == NULL)
         lc_->hash_idx = lc_->hash_entry;
   }
...
{
   if (!lc)
      return 1; // always tring stop iteration
   LwqqClient_* lc_ = (LwqqClient_*)lc;
   LwqqHashEntry* entry = lc_->hash_beg;
   if (entry == lc_->hash_entry) { // if idx point at begin
      for (entry = lc_->hash_entry + HASH_ENTRY_SIZE - 1;
           entry > lc_->hash_entry; --entry) {
                         ...
                         void* data)
{
   ...
   LwqqClient_* lc_ = (LwqqClient_*)lc;
   LwqqHashEntry* entry;
   for (entry = lc_->hash_entry; entry != lc_->hash_entry + HASH_ENTRY_SIZE - 1;
        entry++) {
      if (entry->name == NULL) {
         entry->name = name;
         entry->func = func;
...
void lwqq_hash_set_beg(LwqqClient* lc, const char* hash_name)
{
   ...
   lc_->hash_next = 0;
   if (hash_name == NULL)
      return; // only clear hash_next
   for (entry = lc_->hash_entry; entry != lc_->hash_entry + HASH_ENTRY_SIZE;
        entry++) {
      if (entry->name == NULL)
         break;
