...
static int upload_offline_pic_back(LwqqHttpRequest* req, LwqqMsgContent* c,
                                   const char* to);
...
static TABLE_BEGIN_LONG(unescape, const char*, const char, "")
    TR('\n', "\\\\n");
TR('\r', "\\\\n");
TR('\t', "\\\\t");
TR('\\', "\\\\\\\\");
   ...
   for (ctent = tmp; ctent != NULL; ctent = ctent->next) {
      if (ctent->type == JSON_ARRAY) {
         ...
         if (!strcmp(buf, "font")) {
            ...
            c->data.img.success
                = s_atoi(json_parse_simple_value(ctent, "success"), 0);
            c->data.img.file_path
                = s_strdup(json_parse_simple_value(ctent, "file_path"));
            TAILQ_INSERT_TAIL(&msg->content, c, entries);
            ...
            c->data.cface.name
                = s_strdup(json_parse_simple_value(ctent, "name"));
            if (c->data.cface.name != NULL) {
               c->data.cface.file_id
                   = s_strdup(json_parse_simple_value(ctent, "file_id"));
               c->data.cface.key
                   = s_strdup(json_parse_simple_value(ctent, "key"));
               char* server
                   = s_strdup(json_parse_simple_value(ctent, "server"));
...
static void insert_msg_delay_by_request_content(LwqqRecvMsgList* list,
                                                LwqqMsg* msg)
{
...
static int process_msg_list(LwqqHttpRequest* req, char* serv_id,
...
static int parse_new_msg(json_t* json, LwqqMsg* opaque)
{
   ...
   if (opaque->type == LWQQ_MS_GROUP_MSG) {
      ...
      msg->group.group_code
          = s_strdup(json_parse_simple_value(json, "group_code"));
...
/*
        static void confirm_friend_request_notify(LwqqClient* lc,LwqqBuddy*
   buddy)
        {
        LIST_INSERT_HEAD(&lc->friends,buddy,entries);
...
static int parse_system_message(json_t* json, void* opaque, void* _lc)
{
   ...
   if (strcmp(type, "verify_required") == 0)
      system->type = VERIFY_REQUIRED;
   else if (strcmp(type, "added_buddy_sig") == 0)
      system->type = ADDED_BUDDY_SIG;
   else if (strcmp(type, "verify_pass_add") == 0)
      system->type = VERIFY_PASS_ADD;
   else if (strcmp(type, "verify_pass") == 0)
      system->type = VERIFY_PASS;
   else
      system->type = SYSTEM_TYPE_UNKNOW;

   if (system->type == SYSTEM_TYPE_UNKNOW)
      return 1;
   ...
   if (system->type == VERIFY_REQUIRED) {
      system->verify_required.msg
          = json_unescape(json_parse_simple_value(json, "msg"));
      system->verify_required.allow
          = s_strdup(json_parse_simple_value(json, "allow"));
   } else if (system->type == ADDED_BUDDY_SIG) {
      system->added_buddy_sig.sig
          = json_unescape(json_parse_simple_value(json, "sig"));
...
static int parse_sys_g_msg(json_t* json, void* opaque, LwqqClient* lc)
{
   ...
   if (strcmp(type, "group_create") == 0)
      ...
      add_new_group
          = msg->member_uin ? strcmp(msg->member_uin, lc->myself->uin) == 0 : 1;
      ...
      msg->admin = lwqq__json_get_string(json, "admin_nick");
      msg->is_myself
          = msg->member_uin ? strcmp(lc->myself->uin, msg->member_uin) == 0 : 1;
...
static int parse_file_transfer(json_t* json, void* opaque)
{
   ...
   while (ptr != NULL) {
      ...
      item->file_name
          = json_unescape(json_parse_simple_value(ptr, "file_name"));
...
static int parse_file_message(json_t* json, void* opaque)
{
   ...
   if (strcmp(mode, "recv") == 0)
      ...
      file->mode = MODE_REFUSE;
   else if (strcmp(mode, "send_ack") == 0)
      file->mode = MODE_SEND_ACK;
   ...
   case MODE_RECV:
      ...
      break;
   case MODE_REFUSE:
      file->refuse.cancel_type
          = atoi(json_parse_simple_value(json, "cancel_type"));
      break;
   default:
      break;
   }
...
static int parse_notify_offfile(json_t* json, void* opaque)
{
   ...
   notify->filesize
       = strtoul(json_parse_simple_value(json, "filesize"), NULL, 10);
   return 0;
}
...
static int parse_shake_message(json_t* json, void* opaque)
{
   ...
   shake->reply_ip
       = strtoul(json_parse_simple_value(json, "reply_ip"), NULL, 10);
   return 0;
}
...
static LwqqAsyncEvent* request_content_offpic(LwqqClient* lc, const char* f_uin,
                                              LwqqMsgContent* c)
{
   LwqqHttpRequest* req;
   LwqqErrorCode error;
   ...
   snprintf(
       ...
       d_host, file_path, f_uin, lc->clientid, lc->psessionid);
   s_free(file_path);
   ...
   LwqqHttpRequest* req_cookie
       = lwqq_http_create_default_request(lc, WEBQQ_HOST, 0);
   lwqq_http_set_cookie(req, "p_skey",
                        lwqq_http_get_cookie(req_cookie, "p_skey"), 0);
   lwqq_http_set_cookie(req, "p_uin", lwqq_http_get_cookie(req_cookie, "p_uin"),
                        0);
   lwqq_http_request_free(req_cookie);

   lwqq_http_debug(req, 4);

   return req->do_request_async(req, lwqq__hasnot_post(),
                                _C_(2p_i, set_content_picture_data, req, c));
done:
   lwqq_http_request_free(req);
   return NULL;
}
static LwqqAsyncEvent*
request_content_cface(LwqqClient* lc, const char* group_code,
                      const char* send_uin, LwqqMsgType type, LwqqMsgContent* c)
{
   LwqqHttpRequest* req;
   LwqqErrorCode error;
   LwqqErrorCode* err = &error;
   ...
   snprintf(url, sizeof(url), "%s/"
                              "get_group_pic?type=%d&gid=%s&uin=%s&rip=%s&"
                              "rport=%s&fid=%s&pic=%s&vfwebqq=%s&t=%ld",
            "http://web2.qq.com/cgi-bin", type != LWQQ_MS_GROUP_MSG, group_code,
            send_uin, c->data.cface.serv_ip, c->data.cface.serv_port,
...
static LwqqAsyncEvent* request_content_cface2(LwqqClient* lc, int msg_id,
                                              const char* from_uin,
                                              LwqqMsgContent* c)
{
   LwqqHttpRequest* req;
   ...
   snprintf(url, sizeof(url), "%s/channel/"
                              "get_cface2?lcid=%d&to=%s&guid=%s&count=5&time=1&"
                              "clientid=%s&psessionid=%s",
            WEBQQ_D_HOST, msg_id, from_uin, c->data.cface.name, lc->clientid,
            lc->psessionid);
   req = lwqq_http_create_default_request(lc, url, err);
   req->set_header(req, "Referer", "http://web2.qq.com/webqq.html");
...
static void lwqq_msg_request_picture(LwqqClient* lc, LwqqMsgMessage* msg,
                                     LwqqAsyncEvset** ptr)
{
   ...
   TAILQ_FOREACH(c, &msg->content, entries)
   {
      ...
      if (c->type == LWQQ_CONTENT_OFFPIC
          && list->flags & POLL_AUTO_DOWN_BUDDY_PIC) {
         if (set == NULL)
         ...
         if (msg->super.super.type == LWQQ_MS_BUDDY_MSG)
            event = request_content_cface2(lc, msg->super.msg_id,
                                           msg->super.from, c);
         else {
...
static void lwqq_msg_message_bind_buddy(LwqqClient* lc, LwqqMsgMessage* msg,
                                        LwqqAsyncEvset** ptr)
{
   LwqqAsyncEvset* set = *ptr;
   ...
   const char* serv_id = (type == LWQQ_MS_DISCU_MSG) ? msg->discu.did
                                                     : msg->super.from;
   ...
   for (cur = json_tmp; cur != NULL; cur = cur->previous) {
      ...
      case LWQQ_MT_MESSAGE:
         ...
         if (ret == RET_WELLFORM_MSG) {
            ...
            if (msg->type != LWQQ_MS_SESS_MSG
                && msg->type != LWQQ_MS_GROUP_WEB_MSG)
               lwqq_msg_message_bind_buddy(list->lc, (LwqqMsgMessage*)msg,
                                           &set);
         }
         if (set) {
            lwqq_async_add_evset_listener(
                set, _C_(2p, insert_msg_delay_by_request_content, list, msg));
...
static void insert_recv_msg_with_order(LwqqRecvMsgList* list, LwqqMsg* msg)
{
   ...
   if ((msg->type & LWQQ_MT_BITS) == LWQQ_MT_MESSAGE) {
      ...
      int inserted = 0;
      if (msg_list->last_id == id
          && msg_list->flags & POLL_REMOVE_DUPLICATED_MSG) {
         s_free(rmsg);
         lwqq_msg_free(msg);
         inserted = 1;
      } else {
...
static void receive_poll_message(LwqqHttpRequest* req, char* post)
{
   ...
   req->do_request_async(req, 1, post,
                         _C_(2p, receive_poll_message, req, post));
}
#endif
...
static void* start_poll_msg(void* msg_list)
{
   ...
   snprintf(msg, sizeof(msg),
            "r={\"ptwebqq\":\"%s\","
            "\"clientid\":\"%s\",\"psessionid\":\"%s\",\"key\":\"\""
            "}",
            lc->session.ptwebqq, lc->clientid, lc->psessionid);
   urlencode(msg, 2);
   ...
   req->set_header(req, "Accept-Encoding", "gzip, deflate");
   ...
   try_restart_poll(lc);
#else
   req->do_request_async(req, 1, msg,
                         _C_(2p, receive_poll_message, req, s_strdup(msg)));
#endif
   return NULL;
}
                                      ...
                                      int* has_cface)
{
   ...
   TAILQ_FOREACH(c, &msg->content, entries)
   {
      ...
      case LWQQ_CONTENT_OFFPIC:
         snprintf(buf, sizeof(buf),
                  "[" KEY("offpic") "," KEY("%s") "," KEY("%s") ",%lu],",
                  c->data.img.file_path, c->data.img.name,
      ...
      case LWQQ_CONTENT_CFACE:
         ...
         if (c->data.cface.name) {
            if (msg_type == LWQQ_MS_GROUP_MSG || msg_type == LWQQ_MS_DISCU_MSG)
               snprintf(buf, sizeof(buf),
                        "[" KEY("cface") "," KEY("group") "," KEY("%s") "],",
                        c->data.cface.name);
            else if (msg_type == LWQQ_MS_BUDDY_MSG
                     || msg_type == LWQQ_MS_SESS_MSG)
   ...
   snprintf(
       buf, sizeof(buf),
       "[" KEY("font") ",{" KEY("name") ":" KEY("%s") "," KEY("size") ":" KEY(
...
static LwqqAsyncEvent*
lwqq_msg_upload_offline_pic(LwqqClient* lc, LwqqMsgContent* c, const char* to)
{
   if (c->type != LWQQ_CONTENT_OFFPIC)
      return NULL;
   if (!c->data.img.data || !c->data.img.name || !c->data.img.size)
      return NULL;
   ...
   snprintf(
       url, sizeof(url),
       "http://weboffline.ftn.qq.com/ftn_access/upload_offline_pic?time=%ld",
       LTIME);
   req = lwqq_http_create_default_request(lc, url, &err);
   ...
   lwqq_http_debug(req, 4)
       ...
       req->add_form(req, LWQQ_FORM_CONTENT, "callback",
   ...
   req->add_form(req, LWQQ_FORM_CONTENT, "uin",
                 lc->username); ///<this may error
   ...
   return req->do_request_async(req, lwqq__hasnot_post(),
                                _C_(3p_i, upload_offline_pic_back, req, c, to));
}
static int upload_offline_pic_back(LwqqHttpRequest* req, LwqqMsgContent* c,
                                   const char* to)
{
...
static LwqqAsyncEvent* query_gface_sig(LwqqClient* lc)
{
   ...
   snprintf(url, sizeof(url),
            "%s/get_gface_sig2?clientid=%s&psessionid=%s&t=%ld",
            "https://d.web2.qq.com/channel", lc->clientid, lc->psessionid,
            time(NULL));
   ...
   return req->do_request_async(req, lwqq__hasnot_post(),
                                _C_(p, process_gface_sig, req));
}

static int upload_cface_back(LwqqHttpRequest* req, LwqqMsgContent* c,
                             const char* to)
{
   int ret;
   int err = 0;
...
static LwqqAsyncEvent* lwqq_msg_upload_cface(LwqqClient* lc, LwqqMsgContent* c,
                                             LwqqMsgType type, const char* to)
{
   if (c->type != LWQQ_CONTENT_CFACE)
      return NULL;
   if (!c->data.cface.name || !c->data.cface.data || !c->data.cface.size)
      return NULL;
   ...
   if (type == LWQQ_MS_GROUP_MSG) {
      ...
      req->add_form(req, LWQQ_FORM_CONTENT, "f",
                    "EQQ.Model.ChatMsg.callbackSendPicGroup");
      // cface
      // 上传是会占用自定义表情的空间的.这里的fileid是几就是占用第几个格子.
   ...
   return req->do_request_async(req, lwqq__hasnot_post(),
                                _C_(3p_i, upload_cface_back, req, c, to));
}

static void msg_send_continue(LwqqClient* lc, LwqqMsgMessage* msg,
                              LwqqAsyncEvent* event)
{
...
static void msg_send_delay(LwqqClient* lc, LwqqMsgMessage* msg,
                           LwqqAsyncEvent* event, long delay)
{
...
{
   LwqqGroup* g = NULL;
   if (send->type != LWQQ_MS_GROUP_MSG && send->type != LWQQ_MS_DISCU_MSG)
      return;
...
LwqqAsyncEvent* lwqq_msg_send(LwqqClient* lc, LwqqMsgMessage* msg)
{
   ...
   if (will_upload) {
      ...
      lwqq_async_add_evset_listener(
          evset, _C_(4p, msg_send_delay, lc, msg, event, 5000L));
   ...
   snprintf(data, sizeof(data), "\"msg_id\":%ld,"
                                "\"clientid\":\"%s\","
                                "\"psessionid\":\"%s\"}",
   ...
   LwqqAsyncEvent* ret = req->do_request_async(
...
LWQQ_EXPORT
int lwqq_msg_send_text(LwqqClient* lc, int type, const char* to,
                       const char* message)
{
   if (!lc || !to || !message)
      return 0;
...
int lwqq_msg_check_lost(LwqqClient* lc, LwqqMsg** p_msg, LwqqGroup* g)
{
   LwqqMsg* msg = *p_msg;
   int ret = 0;
   if (msg->type != LWQQ_MS_GROUP_MSG && msg->type != LWQQ_MS_DISCU_MSG)
      return 0;
...
void lwqq_msg_check_member_chg(LwqqClient* lc, LwqqMsg** p_msg, LwqqGroup* g)
{
   ...
   if (g->info_seq == 0)
      return; // doesn't load complete in first time
...
LwqqAsyncEvent* lwqq_msg_refuse_file(LwqqClient* lc, LwqqMsgFileMessage* file)
{
   char url[512];
   snprintf(url, sizeof(url),
            "%s/channel/refuse_file2?"
            "lcid=%d&to=%s&psessionid=%s&count=1&time=%ld&clientid=%s",
            WEBQQ_D_HOST, file->session_id, file->super.from, lc->psessionid,
            time(NULL), lc->clientid);
   LwqqHttpRequest* req = lwqq_http_create_default_request(lc, url, NULL);
   req->set_header(req, "Referer", WEBQQ_D_REF_URL);
...
LWQQ_EXPORT
LwqqAsyncEvent* lwqq_msg_upload_offline_file(LwqqClient* lc,
                                             LwqqMsgOffFile* file,
                                             LwqqUploadFlag flags)
{
   char url[512];
   snprintf(
       url, sizeof(url),
       "http://weboffline.ftn.qq.com/ftn_access/upload_offline_file?time=%ld",
       LTIME);
   LwqqHttpRequest* req = lwqq_http_create_default_request(lc, url, NULL);
   req->set_header(req, "Referer", "http://web2.qq.com/");
   req->set_header(req, "Origin", "http://web2.qq.com");
   ...
   file->req = req;
   return req->do_request_async(req, lwqq__hasnot_post(),
                                _C_(2p_i, upload_offline_file_back, req, file));
}
...
LwqqAsyncEvent* lwqq_msg_send_offfile(LwqqClient* lc, LwqqMsgOffFile* file)
{
   ...
   snprintf(post, sizeof(post), "r={\"to\":\"%s\",\"file_path\":\"%s\","
                                "\"filename\":\"%s\",\"to_uin\":\"%s\","
                                "\"clientid\":\"%s\",\"psessionid\":\"%s\"}",
            file->super.to, file->path, file->name, file->super.to,
            lc->clientid, lc->psessionid);

   return req->do_request_async(req, lwqq__has_post(),
                                _C_(2p_i, send_offfile_back, req, file));
}
                                     ...
                                     LwqqProgressFunc progress, void* prog_data)
{
   ...
   snprintf(
       url, sizeof(url),
       "http://file1.web.qq.com/v2/%s/%s/%ld/%s/%s/1/f/1/0/0?psessionid=%s",
       file->super.from, file->super.to, time(NULL) % 4096, lc->index, lc->port,
       lc->psessionid);
   LwqqHttpRequest* req = lwqq_http_create_default_request(lc, url, NULL);
   req->set_header(req, "Referer", "http://web2.qq.com/");
...
LwqqAsyncEvent* lwqq_msg_input_notify(LwqqClient* lc, const char* serv_id)
{
   ...
   snprintf(
       url, sizeof(url),
       "%s/channel/input_notify2?to_uin=%s&clientid=%s&psessionid=%s&t=%ld",
...
LwqqAsyncEvent* lwqq_msg_shake_window(LwqqClient* lc, const char* serv_id)
{
   ...
   char url[512];
   snprintf(url, sizeof(url),
            "%s/channel/shake2?to_uin=%s&clientid=%s&psessionid=%s&t=%ld",
            WEBQQ_D_HOST, serv_id, lc->clientid, lc->psessionid, LTIME);
   LwqqHttpRequest* req = lwqq_http_create_default_request(lc, url, NULL);
   req->set_header(req, "Referer", WEBQQ_D_REF_URL);
...
LWQQ_EXPORT
LwqqAsyncEvent* lwqq_msg_friend_history(LwqqClient* lc, const char* serv_id,
                                        LwqqHistoryMsgList* list)
{
   if (!lc || !serv_id || !list)
      return NULL;
   char url[512];
   snprintf(url, sizeof(url), "http://web2.qq.com/cgi-bin/webqq_chat/"
                              "?cmd=1&tuin=%s&vfwebqq=%s&page=%d&row=%d&"
                              "callback=alloy.app.chatLogViewer.renderChatLog&"
                              "t=%ld",
            serv_id, lc->vfwebqq, list->page, list->row, time(NULL));
   LwqqHttpRequest* req = lwqq_http_create_default_request(lc, url, NULL);
   req->set_header(req, "Referer", "http://web2.qq.com/");

   return req->do_request_async(
       req, lwqq__hasnot_post(),
       _C_(3p_i, process_msg_list, req, s_strdup(serv_id), list));
}

LwqqAsyncEvent* lwqq_msg_group_history(LwqqClient* lc, LwqqGroup* g,
                                       LwqqHistoryMsgList* list)
{
   if (!lc || !g || !list)
      return NULL;
   char url[512];
   snprintf(url, sizeof(url), "http://cgi.web2.qq.com/keycgi/top/"
                              "groupchatlog?ps=%d&bs=%d&es=%d&gid=%s&mode=1&"
                              "vfwebqq=%s&t=%ld",
            list->row, list->begin, list->end, g->code, lc->vfwebqq,
            time(NULL));
   LwqqHttpRequest* req = lwqq_http_create_default_request(lc, url, NULL);
   req->set_header(req, "Referer",
                   "http://cgi.web2.qq.com/cfproxy.html?v=20110412001&id=2");

   return req->do_request_async(
       req, lwqq__hasnot_post(),
       _C_(3p_i, process_group_msg_list, req, NULL, list));
}

LwqqAsyncEvent* lwqq_msg_remove_uploaded_cface(LwqqClient* lc,
                                               LwqqMsgContent* cface)
{
   if (!lc || !cface || cface->type != LWQQ_CONTENT_CFACE
       || !cface->data.cface.name)
      return NULL;
   char url[256];
   snprintf(url, sizeof(url),
            "http://web2.qq.com/cgi-bin/webqq_app/?cmd=12&bd=%s&vfwebqq=%s",
            cface->data.cface.name, lc->vfwebqq);
   LwqqHttpRequest* req = lwqq_http_create_default_request(lc, url, NULL);
   req->set_header(req, "Referer", "http://web2.qq.com/webqq.html");
