...
typedef struct LwqqMsgFileMessage {
   LwqqMsgSeq super;
   int msg_id;
   enum { MODE_RECV, MODE_REFUSE, MODE_SEND_ACK } mode;
   int session_id;
   time_t time;
   int type;
   char* reply_ip;
   union {
      ...
      struct {
         enum { CANCEL_BY_USER = 1, CANCEL_BY_OVERTIME = 3 } cancel_type;
      } refuse;
   };
   LwqqHttpRequest* req;
} LwqqMsgFileMessage;
...
#define LWQQ_CONTENT_EXT_FILE(url, name, size) lwqq_msg_fill_ext("file", url, name, size, 0)
...
#define lwqq_msg_send_buddy(lc, buddy, message)                                \
   ((buddy != NULL)                                                            \
        ? lwqq_msg_send_simple(lc, LWQQ_MT_BUDDY_MSG, buddy->uin, message)     \
        : NULL)
#define lwqq_msg_send_group(lc, group, message)                                \
   ((group != NULL)                                                            \
        ? lwqq_msg_send_simple(lc, LWQQ_MT_GROUP_MSG, group->gid, message)     \
        : NULL)
...
typedef enum { DONT_EXPECTED_100_CONTINUE = 1 << 0 } LwqqUploadFlag;
